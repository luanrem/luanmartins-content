...
Every `##` and `###` in a body comes out with an `id`, so a link can point at a
section instead of at the top of the page. Nobody writes it — it is derived from
the heading's own text: lowercased, accents dropped, anything that is not a
letter or a digit folded to a hyphen.

```
## As variáveis de ambiente    →    id="as-variaveis-de-ambiente"
```

Two headings with the same text in one file get `-1`, `-2` and so on, in the
order they appear. The id is derived even over one written by hand in raw HTML:
two sources for a section's anchor would be two truths about where it lives.

Because it comes from the text, the same text always produces the same anchor —
but **renaming a heading changes its anchor**, and a link written against the
old one stops working. Links inside the site are regenerated by the same build;
one pasted somewhere else is not.

A heading needs at least one letter or digit. One made only of emoji or
punctuation produces no anchor and fails the build, rather than shipping a
section nothing can link to. Letters outside `a-z` that do not decompose to one
— `ß`, for instance — fold to a hyphen like any other symbol; nothing here is
written in a language where that matters.
